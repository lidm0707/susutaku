...
| Piece | File |
|---|---|
| Tool parse (line + XML) | `backend/src/domain/service/tool_call.rs` (`ToolCall::AgentRun`, `TOOL_AGENT_RUN`, `XML_AGENT_RUN`) |
...
| Port | `backend/src/port/outbound/agent_run.rs` (`trait AgentRun`) |
| Adapter | `backend/src/infra/manager_run.rs` (`ManagerRun` → `Manager::run`) |
| Wiring | `backend/src/main.rs` (`/api/chat`) + `backend/src/api.rs` `zai_chat_deps` (`/api/chat/zai` and `/api/chat/zai/stream`) |
...
- The tool is disabled (tool result is an error string) when the backend runs
  without a manager adapter — e.g. minimal chat-only deployments.
- Only the named agent's slot is touched; the chat's shared sandbox
  (`AgentSandbox::restore()`) is unchanged, so `TOOL: SHELL` keeps its old
  semantics.
