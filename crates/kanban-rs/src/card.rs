...
/// Run-status values persisted on `kanban_cards.run_status`.
...
pub enum Runner {
    ...
    Pinned(String),
    /// The attached pipeline's agent node decides at run time.
    Pipeline,
}
...
impl Runner {
    pub fn as_kind(&self) -> &'static str {
        match self {
            ...
            Runner::Pipeline => RUNNER_PIPELINE,
        }
    }
}
