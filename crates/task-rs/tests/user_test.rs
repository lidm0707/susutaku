...
async fn user_auth_roundtrip() {
    ...
    let short = store
        ...
        .await;
    assert!(matches!(short, Err(task_rs::StoreError::PasswordTooShort)));
...
async fn default_admin_seeded_and_forced_password_change() {
    ...
    let token = store
        .login(task_rs::DEFAULT_ADMIN_USER, task_rs::DEFAULT_ADMIN_PASSWORD)
        .await
        .expect("login")
        .expect("token");
    ...
    let bad = store
        ...
        .await;
    assert!(matches!(bad, Err(task_rs::StoreError::BadCredentials)));

    // Too-short new password is rejected.
    let short = store
        .change_password(user.id, task_rs::DEFAULT_ADMIN_PASSWORD, "short")
        .await;
    assert!(matches!(short, Err(task_rs::StoreError::PasswordTooShort)));
