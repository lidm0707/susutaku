//! Infrastructure: Postgres adapter implementing the task outbound ports
//! over `task_rs::Store`.
...
use task_rs::{
    ...
    NewSkill, ProjectRow, ResourceRow, RunRecordNew, RunRecordRow, SkillRow, Store, StoreError,
    UpdateCard, UpsertResource, WorkspaceRow,
};

use crate::domain::{AgentConfigDraft, CardMove, CardPatch, NewCard, NewProject, NewWorkspace};
use crate::port::outbound::{
    AgentConfigRepo, CardRepo, CardTx, CommentRepo, CommentTx, ProjectRepo, ResourceRepo,
    SkillRepo, WorkspaceRepo,
};
...
/// Postgres-backed implementation of every task repo port.
pub struct PgTask {
    store: Arc<Store>,
}
