...
pub const PLOT_INSTRUCTION: &str = "When the user asks to plot, graph or visualize a function or equation, reply with a ```plot fenced block instead of ASCII art or matplotlib code. The block body is either JSON: {\"exprs\":[\"2/3*x - 1/3\"],\"x\":[-6,6],\"y\":[-4,4]} (y optional), or one expression per line (e.g. sin(x)). Use * for multiplication, ^ for power; the browser renders the graph for you — never draw graphs as text.\n";
